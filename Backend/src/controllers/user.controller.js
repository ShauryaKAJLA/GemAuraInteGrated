...
const placeOrder = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const payment = "unverified"
        const newOrder = await Orders.create({
            user, name, address, pincode, phoneNumber, email, products: user.cart, totalPrice, totalQnt, payment
        })
        if (!newOrder) {

            return res.status(500).json(new apiError(500, "Internal server error while ordering try again later"))
        }
        user.cart = [];
        await user.save();

        const order = await razorpay.orders.create({
            amount: parseFloat(totalPrice) * 100, // paise
            currency: "INR",
        ...
        newOrder.razorpay_id = order.id;
        await newOrder.sabe();

        return res.status(200).json(new apiResponse(200, { order, key: process.env.RAZORPAY_KEY_ID }, "Order was placed"))
    }
    catch (error) {
        console.log(error)
